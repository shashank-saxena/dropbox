...
function fileHandler() {
  ...
  return {
    "uploadFile": function(filePath, fileObj) {
        try {
            if (!filePath || !filePath) {
                throw(new Error('filePath or fileObj is must!'));
            }

            // File is smaller than 150 Mb - use filesUpload API
            if (fileObj.size < API_UPLOAD_FILE_SIZE_LIMIT) { 
                return uploadFile(filePath, fileObj)
    ...
    "getFile": function(fileId, filePath) {
        try{
            if (!fileId || !filePath) {
                throw(new Error('file ID or file path is must!'));
            }
            return downloadFile(fileId, filePath);
        } catch(err) {
            throw(new Error(err));
        }
    },
